...
class AccountInitiatePage extends StatefulWidget {
  ...
  @override
  _AccountInitiatePageState createState() => _AccountInitiatePageState();
}

class _AccountInitiatePageState extends State<AccountInitiatePage> {
    ...
    if (_authProvider!.authState.progressState == 3) {
      ...
      if (account.contains('@')) {
        ...
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (BuildContext context) => AccountVerificationPage(
              mobile: accountController.text.trim(),
              callback: widget.callback,
            ),
          ),
        );
  ...
  _onSendPressed() async {
    ...
    if (account.contains('@')) {
      _authProvider!.initiateEmailVerification(email: accountController.text.trim());
    } else {
      _authProvider!.initiateMobileVerification(mobile: accountController.text.trim());
    }
  }
}
